...
看不到 `friday` MCP 工具，或调用返回 401/403，引导用户运行 `npx -y @friday-ai-codes/mcp setup`（见 `friday` 技能「环境未就绪」一节）。此外：
...
按 阶段一 → `search_learning_cases` → 阶段二 → 阶段三 → `create_learning_case`（记录结果、根因、解法、已验证测试，见 `friday-memory`）顺序跑完，全程共用第一个 `run_id`。
